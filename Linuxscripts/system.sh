...
echo "It's "$(date)" Free Memory: "$(free | grep -i 'mem' | awk '{print $4}') > ~/backups/freemem/free_mem.txt

# Disk usage output to a disk_usage.txt file
echo "It's "$(date)" Disk Usage: "$(df -H | head -2 | grep -i 'udev' | awk '{print $5}') > ~/backups/diskuse/disk_usage.txt

# List open files to a open_list.txt file
lsof > ~/backups/openlist/open_list.txt

# Free disk space to a free_disk.txt file
echo "It's "$(date)", Free Disk: "$(df -H | head -2 | grep -i 'udev' | awk '{print $4}') > ~/backups/freedisk/free_disk.txt
